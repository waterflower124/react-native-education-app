...
export default class GamesScreen extends Component {
    ...
    getGames = async() => {
        try {
            ...
            var uri = Global.BASE_URL + "/api/games" + query;
                                                
            console.log(TAG + " callGetGamesAPI uri " + uri);

            WebService.callServiceGet(uri, this.handleGetGamesAPI);
        } catch (error) {
            console.log(error)
            this.setState({
                loading: false,
    ...
    handleGetGamesAPI = async(response, isError) => {
        console.log(TAG + " callGetGamesAPI Response " + JSON.stringify(response));
        console.log(TAG + " callGetGamesAPI isError " + isError);
        try {
            if(!isError) {
                if(response.status == "success") {
                    if(this.state.refreshing) {
                        this.setState({
                            game_array: response.games
                        })
                    } else {
                        this.setState({
                            game_array: [...this.state.game_array, ...response.games]
                        })
                    }
                    if(response.games.length < this.state.limit) {
                        this.setState({
                            more_load: false
                        })
                    } else {
    ...
    openPopUp = async(item) => {
        if(item == null) {
            ...
            this.setState({
                ...
                selected_link: item.link,
                selected_upload_image: ''
            })
        }
        this.setState({add_newgame_popup: true})
    }

    addNewGame = async() => {
        if(this.state.selected_item == null) {
            if(this.state.selected_upload_image == '') {
                Alert.alert(Constants.warnning, "Please select image");
                return;
            }
        }
    ...
    deleteGame = async(item) => {
        try {
            ...
            var uri = Global.BASE_URL + "/api/game/" + item.id;
                                                
            console.log(" callDeleteGameAPI uri " + uri);

            WebService.callServiceDelete(uri, this.handleDeleteGameAPI);
        } catch (error) {
            console.log(error)
            this.setState({
                loading: false,
    ...
    handleDeleteGameAPI = async(response, isError) => {
        console.log(" callDeleteGameAPI Response " + JSON.stringify(response));
        console.log(" callDeleteGameAPI isError " + isError);
        try {
            if(!isError) {
                if(response.success) {
                    var game_array = this.state.game_array;
                    for(var i = 0; i < game_array.length; i ++) {
                        if(game_array[i].id == this.state.selected_item.id) {
                            game_array.splice(i, 1);
                            break;
                        }
                    }
                    this.setState({
    ...
    render() {
        return (
                ...
                />
            {
                    ...
                    <View style = {{width: '100%', height: '100%', alignItems: 'center', justifyContent: 'center'}}>
                        <View style = {{width: '90%', borderRadius: 10, overflow: 'hidden'}}>
                            ...
                            <View style = {{width: '100%', paddingHorizontal: 15, paddingVertical: 15, backgroundColor: Colors.primary}}>
                                <Text style = {{fontSize: 16, color: Colors.white}}>{this.state.selected_item == null ? "Add New Game" : "Edit Game"}</Text>
                            </View>
                ...
                {
                    !this.state.loading && this.state.game_array.length == 0 &&
                    <View style = {{flex: 1, alignItems: 'center', justifyContent: 'center'}}>
                        <Text style = {{fontSize: 18, color: Colors.primary}}>{"There is no games"}</Text>
                    </View>
                }
                    <FlatList
                        ...
                        data={this.state.game_array}
                        extraData={this.state}
                        showsVerticalScrollIndicator={false}
                        keyExtractor={(item, index) => index.toString()}
                        onMomentumScrollBegin={() => { this.onEndReachedCalledDuringMomentum = false; }}
                        renderItem={({ item, index }) => <GameCard key = {index} style = {{marginTop: 15}} data = {item} navigation = {this.props.navigation} type = {"games_screen"} deleteGame = {this.deleteGamePopUp} editGame = {this.openPopUp}/>}
                    >
                    </FlatList>
                </View>
            {
                ...
                <View style = {{position: 'absolute', bottom: 20, right: 20, }}>
                    <TouchableOpacity style = {{width: 60, height: 60, borderRadius: 30, backgroundColor: Colors.secondary, alignItems: 'center', justifyContent: 'center'}} onPress = {() => this.openPopUp(null)}>
